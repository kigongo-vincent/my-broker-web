...
//   UploadWorkerProgressMsg,
//   UploadWorkerRequest,
// } from "./imageUpload.worker";
...
//   if (!worker) {
//     worker = new Worker(new URL("./imageUpload.worker.ts", import.meta.url), {
//       type: "module",
//     });
//   }
//   return worker;
...
//   return useMutation({
//     mutationFn: (input: UploadImagesInput): Promise<PostAsset[]> => {
...
//         const handleMessage = (
//           e: MessageEvent<
//             UploadWorkerProgressMsg | UploadWorkerDoneMsg | UploadWorkerErrorMsg
//           >
//         ) => {
//           const msg = e.data;
...
//             progressMap[msg.index] = msg.pct;
//             input.onProgress?.({ ...progressMap });
//           } else if (msg.type === "done") {
//             w.removeEventListener("message", handleMessage);
//             resolve(msg.assets as PostAsset[]);
...
//         const req: UploadWorkerRequest = {
//           type: "upload",
//           jobId,
//           images: input.images.map((file, index) => ({ file, index })),
...
/* ------------------------------------------------------------------ */
/* Single-file compression + upload (main thread).                     */
/*                                                                       */
/* Used for one-off, single-image flows (e.g. avatar / ID photo         */
/* upload) where spinning up the worker isn't worth it. Bulk post-photo */
/* uploads use the worker path below (useUploadImages) instead — that   */
/* one never calls these directly.                                      */
/* ------------------------------------------------------------------ */
 ...
 * Three-tier compression:
 *  1. jSquash (WASM MozJPEG/WebP codecs) — best ratio, needs WASM support.
 *  2. browser-image-compression (canvas + worker) — works on virtually
 *     every phone, including older Android WebViews where WASM is
 *     flaky/slow.
 *  3. Original file, untouched — last resort so the upload can still
 *     proceed.
 */
export async function compressImage(
  file: File,
...
/**
 * Unsigned Cloudinary upload with XHR (so we get real progress events —
 * fetch() has no upload-progress API on the main thread).
 */
export function uploadToCloudinary(
  blob: Blob,
  folder: string,
...
/* ------------------------------------------------------------------ */
/* Feed / infinite feed / details / geo                                */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Image upload (worker-backed: compression + Cloudinary upload happen */
/* off the main thread — see imageUpload.worker.ts).                   */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Create post — called once on final submit, using assets that were   */
/* already uploaded via useUploadImages.                               */
/* ------------------------------------------------------------------ */
...
export function useCreatePost() {
  ...
  return useMutation({
    mutationFn: async (input: PostI) => {
      return await Post<PostI, unknown>("/posts", input);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["posts"] });
    },
